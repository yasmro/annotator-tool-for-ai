...
function EditorContent() {
  ...
  const { dragSelection, handleWheel, handleCanvasMouseDown, handleCanvasMouseMove, handleCanvasMouseUp } =
    ...
    <div className="flex h-screen flex-col bg-background">
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="relative flex flex-1 flex-col">
          ...
          {project.imageDataUrl && (
            <div className="flex items-center gap-2 border-b bg-muted/30 p-2">
              ...
              <div className="flex items-center gap-1">
                ...
                <Button variant="outline" size="icon" className="h-8 w-8 bg-transparent" onClick={() => setZoom(1)}>
              ...
              <Button
                variant={showAnnotationList ? "secondary" : "outline"}
                size="sm"
          ...
          {/* Canvas */}
          <div className="flex-1 overflow-auto bg-muted/20" onWheel={handleWheel}>
            {!project.imageDataUrl ? (
              <div
              ...
              <div
                ...
                style={{ cursor: dragSelection.isDrawing ? "crosshair" : "default" }}
                    ...
                    return (
                      <Rnd
                        ...
                        onDragStop={(_e, d) => {
                          ...
                          const newAbsX = d.x / project.imageNaturalWidth
                          const newAbsY = d.y / project.imageNaturalHeight
                          updateAnnotationPosition(annotation.id, newAbsX, newAbsY)
                        }}
                        onResizeStop={(_e, _direction, ref, _delta, position) => {
                          const newAbsW = Number.parseFloat(ref.style.width) / project.imageNaturalWidth
                          const newAbsH = Number.parseFloat(ref.style.height) / project.imageNaturalHeight
                          const newAbsX = position.x / project.imageNaturalWidth
                          const newAbsY = position.y / project.imageNaturalHeight
                          updateAnnotationSize(annotation.id, newAbsX, newAbsY, newAbsW, newAbsH)
                        }}
                        onMouseDown={(e) => {
                          e.stopPropagation()
                        ...
                        style={{
                          ...
                          backgroundColor:
                            project.selectedId === annotation.id ? `${annotation.color}33` : `${annotation.color}11`,
                          pointerEvents: "auto",
                          cursor: "move",
                          zIndex: depth,
                        }}
                      >
                        <div className="absolute left-1 top-1 rounded bg-black/70 px-1 py-0.5 text-xs text-white">
                          ...
                          {isLayoutType(annotation.layoutType) && (
                            <span className="ml-1 opacity-70">({annotation.layoutType})</span>
                          )}
                        </div>
                      </Rnd>
                    )
                ...
                {getHierarchicalAnnotations().map(({ annotation, depth }) => (
                  <button
                    ...
                    className={`w-full text-left px-2 py-1.5 rounded text-sm transition-colors ${
                      project.selectedId === annotation.id ? "bg-primary text-primary-foreground" : "hover:bg-muted"
                    }`}
                  ...
                  >
                    ...
                    {annotation.componentKind}
                    {isLayoutType(annotation.layoutType) && (
                      <span className="ml-1 text-xs opacity-70">({annotation.layoutType})</span>
                    )}
                  </button>
                ))}
                ...
                {selectedAnnotation ? (
                  <div className="space-y-4 p-4">
                    <div>
                      ...
                      <p className="font-mono text-sm">{selectedAnnotation.id}</p>
                    </div>
                            ...
                            .map((a) => (
                              <SelectItem key={a.id} value={a.id}>
                                <div className="flex flex-col py-1">
                                  ...
                                  <span className="text-sm">{a.layoutType}</span>
                                </div>
                              </SelectItem>
                            ))}
                        </SelectContent>
                        ...
                        {ANNOTATION_COLORS.map((color) => (
                          <button
                            ...
                            className={`h-6 w-6 rounded border-2 ${
                              selectedAnnotation.color === color ? "border-foreground" : "border-transparent"
                            }`}
                            style={{ backgroundColor: color }}
                            onClick={() => updateAnnotationField(selectedAnnotation.id, "color", color)}
                          />
                        ))}
                      </div>
                    </div>
                      ...
                      <div className="space-y-3 rounded-md border p-3">
                          ...
                          <Select
                            ...
                            onValueChange={(value) => {
                              const newFlexLayout = {
                                ...
                                justify: selectedAnnotation.flexLayout?.justify || "start",
                                align: selectedAnnotation.flexLayout?.align || "start",
                                gap: selectedAnnotation.flexLayout?.gap || 0,
                              }
                              updateAnnotationField(selectedAnnotation.id, "flexLayout", newFlexLayout)
                            }}
                          >
                        ...
                        <div>
                          ...
                          <Select
                            ...
                            onValueChange={(value) => {
                              const newFlexLayout = {
                                direction: selectedAnnotation.flexLayout?.direction || "row",
                                justify: value as "start" | "center" | "end" | "between" | "around",
                                align: selectedAnnotation.flexLayout?.align || "start",
                                gap: selectedAnnotation.flexLayout?.gap || 0,
                              }
                              updateAnnotationField(selectedAnnotation.id, "flexLayout", newFlexLayout)
                      ...
                      <div className="space-y-3 rounded-md border p-3">
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) => {
                              ...
                              updateAnnotationField(selectedAnnotation.id, "gridLayout", newGridLayout)
                            }}
                            className="h-8"
                          />
                        </div>
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) => {
                              const newGridLayout = {
                                columns: selectedAnnotation.gridLayout?.columns || 1,
